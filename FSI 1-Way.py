...
x_EA_root = x_LE_root + (x_TE_root - x_LE_root)*0.5
y_EA_root = y_LE_root + (y_TE_root - y_LE_root)*0.5            # EA placed at 0.5^chord
z_EA_root = z_LE_root + (z_TE_root - z_LE_root)*0.5

x_EA_tip = x_LE_tip + (x_TE_tip - x_LE_tip)*0.5
y_EA_tip = y_LE_tip + (y_TE_tip - y_LE_tip)*0.5                # EA placed at 0.5^chord
z_EA_tip = z_LE_tip + (z_TE_tip - z_LE_tip)*0.5
...
'''
U_disp, load_vector = FEM.Solve_Displacements(col_index, panel_lift_forces, VLM.M, VLM.N)
...
if max_lift > 0:  
    arrow_scale = 0.001  # Arrow Size Scale
    ...
    ax.quiver(col_x_plot, col_y_plot, col_z_plot, u, v, w, length=1, normalize=False, color='red', label='Panel Lift Forces')
...
forces = load_vector[0::3]
torques = load_vector[1::3]
moments = load_vector[2::3]

fig, ax = plt.subplots(figsize=(10, 4))

# Plot beam nodes as a straight line on X
ax.plot(FEM.y_nodes, np.zeros_like(FEM.y_nodes), color='black', linewidth=2, label='Beam Axis')
ax.scatter(FEM.y_nodes, np.zeros_like(FEM.y_nodes), color='blue', marker='o', label='Nodes')

forces_at_nodes = load_vector[0::3] # Extract forces at each node
max_force = np.max(np.abs(forces_at_nodes))

if max_force > 0:

    arrow_scale = 0.2 
    arrow_lengths = arrow_scale * forces_at_nodes / max_force

    ax.quiver(FEM.y_nodes, np.zeros_like(FEM.y_nodes), np.zeros_like(FEM.y_nodes), arrow_lengths, scale=1, color='red', width=0.001, label='Forces')

ax.set_xlabel('X Position (m)')
ax.set_ylabel('Force (scaled)')
ax.set_title('Beam Nodes and Forces')
ax.legend()
ax.grid(True)
plt.show()
